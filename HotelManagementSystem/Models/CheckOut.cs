    ...
    public class CheckOut
    {
        ...
        public void CheckOutFromRoom()
        {
            ...
            string queryString = @"BEGIN TRANSACTION
                                   INSERT INTO master.[dbproject].[CheckOut] ([checkOutDate],[roomId],[checkOutBy],[totalCharge])
                                    SELECT @checkOutDate ,r.id, ci.checkInBy, SUM(NULLIF(e.charge, 0))
                                        FROM master.[dbproject].[Room] r JOIN master.[dbproject].CheckIn ci ON(r.id = ci.roomId)
																  JOIN master.[dbproject].[OccupancyDates] od ON (od.inDate = ci.checkInDate AND od.roomId = r.id)
																  LEFT JOIN master.[dbproject].[Expense] e ON (e.expenseOf = r.id AND od.inDate <= e.dateOfExpense AND @checkOutDate >= e.dateOfExpense)										
        ...
        public void GenerateReceipt()
        {
            ...
            string queryString = @"SELECT e.description, e.dateOfExpense, e.charge
                                   FROM master.dbproject.Expense e
                                   WHERE expenseOf = (SELECT id FROM master.[dbproject].Room WHERE roomNumber = @roomNumber)";
